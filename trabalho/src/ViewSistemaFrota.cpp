...
void ViewSistemaFrota::cadastroVeiculo(){
    ...
    while ( ! ( std::cin >> valorVenda ))
    {
        std::cin.clear();
        std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
        std::cout << "Valor de venda invalido. Digite novamente\n";
        std::cout << "- Valor de Venda (em reais, entre 1000 e 800000 reais): ";
    }
    ...
    try{
        ...
        if(tipoVeiculo == EnumTipoVeiculo::MOTO){
            ...
            while ( ! ( std::cin >> capacidade ))
            {
                std::cin.clear();
                std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
                std::cout << "Capacidade Invalida. Digite novamente\n";
                std::cout << "- Capacidade da Van (entre 1 e 2000 kg): ";
            }
...
void ViewSistemaFrota::cadastroMotorista(){
    ...
    while ( ! ( std::cin >> idade ))
    {
        std::cin.clear();
        std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
        std::cout << "Idade Invalida. Digite novamente\n";
        std::cout << "- Idade (18 a 120 anos): ";
    }
    ...
    while ( ! ( std::cin >> cargaHoraria ))
    {
        std::cin.clear();
        std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
        std::cout << "Carga Horaria Invalida. Digite novamente\n";
        std::cout << "- Carga Horaria Mensal (entre 1 e 250 horas): ";
    }    

    std::cout << "- Valor por Hora (entre 1 e 1000 reais por hora): ";
    while ( ! ( std::cin >> valorHora ))
    {
        std::cin.clear();
        std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
        std::cout << "Valor por Hora Invalido. Digite novamente\n";
        std::cout << "- Valor por Hora (entre 1 e 1000 reais por hora): ";
    }    
